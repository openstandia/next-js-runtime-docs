...
Deno はデフォルトでセキュアだという特徴を持ちます。特に有効にしない限り、Deno で実行されるプログラムは、ファイルシステムへのアクセス、ネットワーク接続、環境アクセスなどの機密性の高い API にはアクセスできません。これらのリソースへのアクセスは、コマンドラインオプションまたはランタイムパーミッションプロンプトで明示的に許可する必要があります。
